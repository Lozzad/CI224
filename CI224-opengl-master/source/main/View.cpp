...
int View::initialise() {
	...
	// blue? background
	glClearColor(0.3f, 0.4f, 0.6f, 0.0f);
	...
	//colors 
	glGenBuffers(2, model->getPlayer()->getColorBuffer());
	...
	//colors 
	glGenBuffers(2, model->getBoard()->getColorBuffer());
...
void View::update() {
		...
		//1st attribute buffer is player verts
		glEnableVertexAttribArray(0);
		glBindBuffer(GL_ARRAY_BUFFER, *model->getPlayer()->getVertexBuffer());
		glVertexAttribPointer(
			0,			//key, must be same as vertex shader
			3,			//size
			GL_FLOAT,	//type
			GL_FALSE,	//normalised?
			0,			//stride
